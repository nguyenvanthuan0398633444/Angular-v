...
@Component({
  selector: 'ait-job-detail',
  templateUrl: 'job-detail.component.html',
  styleUrls: ['./job-detail.component.scss']
})

export class JobDetailComponent extends AitBaseComponent implements OnInit {
  loading = false;
  comma = '、';
  constructor(
  ...
  ngOnInit() {
    this.route.queryParams
      .subscribe(params => {
        ...
        this.setState({ company_key }, 'state');
        this.getCompanyInfo();
      }
      );
  ...
  getCompanyInfo = () => {
    ...
    this.compService.findCompanyByListKeys([this.state.company_key]).then(res => {
      console.log(res)
  ...
  getJobInfo = () => {
    this.loading = true;
    this.jobService.findJobByCompanys(this.listCompanyKey).then(r => {
      // // // console.log(r)
      if (r.status === RESULT_STATUS.OK) {
        ...
        this.dataJobInfo = this.sortJobsByGroup(
          [...data.map(m => ({ ...m, isOpen: m?._key === this.job_key, field: m?.status?.value ? m?.status?.value : '一時中止' }))]);
        setTimeout(() => {
          const element = document.getElementById(this.job_key);
